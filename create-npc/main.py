...
The output should be in JSON format, with the keys 'name' and 'kind'. 
...
chat_instruction = """You are a {kind}, your name is {name},
...
def generate_character(species: str, ollama_url: str, model: str) -> Character:
    ...
    character = Character(name=character_data['name'], kind=character_data['kind'])
...
def generate_description(character: Character, ollama_url: str, model: str) -> str:
    ...
    user_content = "Create a {kind} with this name: {name}".format(kind=character.kind, name=character.name)
...
def chat_with_character(character: Character, description: str,ollama_url: str, model: str):
    ...
    instructions = chat_instruction.format(kind=character.kind, name=character.name)
